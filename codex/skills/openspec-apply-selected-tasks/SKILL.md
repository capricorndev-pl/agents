...
Prefer this skill over `openspec-apply-change` when the request names task numbers, a section, a range, or a narrow subset of work.
...
1. **Resolve the change**
   ...
   - if still ambiguous, run `openspec list --json` and ask one concise plain-text question
...
3. **Resolve the selected tasks**
   ...
   - only use text matching as a fallback, and confirm if there is more than one plausible match

   If no task selection can be resolved, ask one concise plain-text question and stop until clarified.
...
4. **Read only the focused context**
   ...
   - use targeted search in `tasks.md`, `design.md`, and relevant `specs/**/*.md`
...
6. **Implement only the selected tasks**
   ...
   Work through the selected tasks in order.
   ...
   - update only that task's checkbox in `tasks.md` when it is truly complete

   Do **not** continue to unrelated pending tasks after the selected subset is done.

   Pause if:
   - the task is ambiguous
   - the task uncovers a design problem
...
- If a selector is ambiguous, ask before editing.
- If selected tasks are already complete, say so and stop.
- If the user did not select tasks and wants broad implementation, use `openspec-apply-change` instead.
