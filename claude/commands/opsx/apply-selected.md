---
name: "OPSX: Apply Selected"
description: Implement only selected tasks from an OpenSpec change (Experimental)
category: Workflow
...
Prefer this command over `/opsx:apply` when the request names task numbers, a section, a range, or a narrow subset of work (e.g., `1.1`, `2.3-2.5`, `section 5`).

**Input**: A change name is optional if it can be inferred. A task selection is required unless it can be inferred from the user's request (e.g., `/opsx:apply-selected add-auth 1.1-1.3`).
...
1. **Resolve the change**
   ...
   Always announce: "Using change: <name>" and how to override (e.g., `/opsx:apply-selected <other> <selectors>`).
...
2. **Get the apply data**
   ...
   **Handle states:**
   ...
   - If `state: "all_done"`: report that everything is already complete and stop
   - Otherwise: proceed
...
- If the user did not select tasks and wants broad implementation, use `/opsx:apply` instead.
- Pause on errors, blockers, or unclear requirements - don't guess.
- Use contextFiles from CLI output, don't assume specific file names.

**Fluid Workflow Integration**

This command supports the "actions on a change" model and can be invoked anytime:
