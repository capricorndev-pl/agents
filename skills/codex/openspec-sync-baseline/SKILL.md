...
description: Sync the current implemented project state back into baseline OpenSpec specs. Use when a repository already has code, docs, APIs, or runtime behavior that may have drifted from `openspec/specs`, and Codex needs to compare reality to the baseline, identify stale or missing requirements, and update baseline specs to reflect current behavior without introducing future intent.
---
